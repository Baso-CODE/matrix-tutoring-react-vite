...
const LesPrivateMahasiswaKecamatan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const getHero = await getHeroData("MAHASISWA", "kecamatan"); // Pastikan kategori dan jenis sudah benar di backend
        setHeroData(getHero);
      } catch (error) {
  ...
  const canonicalUrl = `${baseUrl}/les-privat-mahasiswa/${kotaSlug}/${kabupatenSlug}/${kecamatanSlug}`;
