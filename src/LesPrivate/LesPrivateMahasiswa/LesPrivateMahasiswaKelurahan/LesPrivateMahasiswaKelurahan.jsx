...
const LesPrivateMahasiswaKelurahan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responseKelurahan = await getKelurahanBySlug(kelurahanSlug); // Pastikan fungsi ini ada dan benar
        setKelurahan(responseKelurahan.data);

        const getHero = await getHeroData("MAHASISWA", "kelurahan"); // Pastikan kategori dan jenis sudah benar di backend
        setHeroData(getHero);
      } catch (error) {
  ...
  const canonicalUrl = `${baseUrl}/les-privat-mahasiswa/${kotaSlug}/${kabupatenSlug}/${kecamatanSlug}/${kelurahanSlug}`;
      ...
      "@graph": [
        {
          ...
          "itemListElement": [
            ...
            },
            {
              ...
              "position": 5,
              "item": {
                "@id": "${baseUrl}/les-privat-mahasiswa/${kotaSlug}/${kabupatenSlug}/${kecamatanSlug}", 
                "name": "Les Privat Mahasiswa di ${kecamatanSlug
                  .replace(/-/g, " ")
                  .split(" ")
                  .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
                  .join(" ")}"
...

        {
          ...
          "description": "Tutor profesional untuk Les Privat Mahasiswa datang ke rumah dan online di **Kelurahan ${nameCountry}**. Solusi tepat untuk bimbingan mata kuliah sulit dan tugas akhir.",
          "url": "${canonicalUrl}",
...

        {
          ...
          "url": "${canonicalUrl}",
          "name": "Les Privat Mahasiswa Datang ke Rumah di Kelurahan ${nameCountry} | Matrix Tutoring",
          "inLanguage": "id-ID",
          "description": "Raih IPK memuaskan dengan bimbingan personal Les Privat Mahasiswa di Kelurahan ${nameCountry}. Kami mencocokkan Anda dengan tutor yang ahli di bidang studi Anda.",
          "isPartOf": { "@id": "${baseUrl}" },
          "about": { "@id": "${canonicalUrl}#organization" },
          "mainEntity": {
            "@type": "Service",
            "name": "Les Privat untuk Mahasiswa di Kelurahan ${nameCountry}",
...

        {
          ...
          "mainEntity": [
            {
              "@type": "Question",
              "name": "Apakah Les Privat Mahasiswa di Kelurahan ${nameCountry} bisa diadakan di kampus?",
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "Ya, kami menawarkan fleksibilitas lokasi, baik di rumah mahasiswa, perpustakaan, atau lokasi kampus yang disepakati, selama berada di area Kelurahan ${nameCountry}."
              }
            },
            {
              "@type": "Question",
              "name": "Berapa lama durasi satu sesi les privat untuk mahasiswa?",
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "Durasi standar satu sesi adalah 90 hingga 120 menit, namun ini dapat disesuaikan berdasarkan kebutuhan fokus mata kuliah atau tugas yang sedang dikerjakan."
              }
            },
            {
              "@type": "Question",
              "name": "Bagaimana jaminan kualitas tutor Les Privat Mahasiswa?",
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "Kami hanya merekrut tutor dengan latar belakang pendidikan tinggi (S2/S3) atau pengalaman mengajar di universitas, memastikan mereka memiliki pemahaman yang mendalam tentang kurikulum perguruan tinggi."
              }
            }
          ]
        }
