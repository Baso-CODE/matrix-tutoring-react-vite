...
const LesPrivateSBMPTNKabupaten = () => {
  ...
  const programName = "SNBT (SBMPTN)"; // Nama program yang digunakan
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const getHero = await getHeroData("SBMPTN", "kabupaten"); // Gunakan kategori "SBMPTN" atau "SNBT" sesuai backend Anda
        setHeroData(getHero);
      } catch (error) {
  ...
  const canonicalUrl = `${baseUrl}/les-privat-sbmptn/${kotaSlug}/${kabupatenSlug}`;
